...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (!lifecoach)
      return res.status(404).send({ error: "life coach does not exist" });
    ...
    res.json({
      msg: "life coach updated successfully",
      data: updatedlifecoach
    });
  } catch (error) {
    // We will be handling the error later
    console.log(error);
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.json({
      msg: "life coach was deleted successfully",
      data: deletedlifecoach
    });
  } catch (error) {
    // We will be handling the error later
    console.log(error);
